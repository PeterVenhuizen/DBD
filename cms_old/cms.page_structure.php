...
<html>
	<head>
		...
		<script src='js/page_structure.js'></script>
	</head>
...
?> 	
		...
		<section id="cmswrapper">
			...
			<div id="vcup_referal">Voor de Veluwecup pagina structuur ga naar <a href="cms.vcup_structure.php">hier</a>.</div>
		...
		<?php
			$menu_items = mysql_query("SELECT * FROM menu ORDER BY _order");
			while ($item = mysql_fetch_array($menu_items)) {
				...
				if ($item['expand'] == 1) {
					...
					$subs = mysql_query("SELECT * FROM subpages WHERE parent_id = $id ORDER BY _order ASC");
					...
					while ($sub_item = mysql_fetch_array($subs)) {
						echo '	<li><a href="cms.page_structure.php?editID=' . $sub_item['id'] . '" class="lnkSubpage">' . $sub_item['name'] . '</a></li>';
					}
					echo '	</ul>';
				} else {
		...
		?>
			...
			<form action="" method="POST" id="editMenuItem">
				...
				<?php
					if (isset($_POST['editMItem'])) {
						...
						$query = "UPDATE menu SET name='$menu_name', _order='$menu_order', expand='$menu_expand', url='$menu_url' WHERE id='$id'";
						mysql_query($query) or die();
						echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=cms.page_structure.php">';	
					}
				?>
				...
				<?php
					if (isset($_POST['createMenuItem'])) {
						...
						$query = "INSERT INTO menu (name, _order, expand, url) VALUES ('$pagetitle', '$order', '$expand', ''$url)";
						mysql_query($query) or die();
						echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=cms.page_structure.php">';							
					}
				?>					
			...
			<form action="" method="POST" id="createPage">
				...
				<?php
					if (isset($_POST['addPage'])) {
						...
						$query = "INSERT INTO subpages (parent_id, _order, name, published) VALUES ('$parent_id', '$order', '$pagetitle', 0)";
						mysql_query($query) or die();
						echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=cms.page_structure.php">';
					}
				?>
			...
			<form action="" method="POST" id="subPageEdit">
				...
				<script> $('#subPageEdit').show(); 
				</script>
				
				<?php 
					if (isset($_GET['editID'])) {
						$id = mysql_real_escape_string($_GET['editID']);
						$query = mysql_query("SELECT * FROM subpages WHERE id = '$id' LIMIT 1");
						...
						while ($page = mysql_fetch_array($query)) {		
							echo '	<h2 class="h2header">Wijzig subpagina</h2>		
									...
									<input type="text" name="subTitle" id="subTitle" placeholder="E.g. Bestuur" value="' . $page['name'] . '" />
				...
				<?php		
						...
						if (isset($_POST['deleteSubpage'])) {
							...
							$query = "DELETE FROM subpages WHERE id = '$id'";
							mysql_query($query) or die();
							echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=cms.page_structure.php">';	
						}
						...
						if (isset($_POST['submitSubpage'])) {
							...
							$query = "UPDATE subpages SET _order='$order', name='$title', published=$isPublished, content='$content' WHERE id=$id";
							mysql_query($query) or die();
							echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=cms.page_structure.php">';										
						}						
					}
