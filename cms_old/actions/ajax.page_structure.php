<?php 
	...
	if (isset($_POST['menuID'])) {
		...
		$query = mysql_query("SELECT * FROM menu WHERE id = '$id' LIMIT 1");
		...
		while ($menu = mysql_fetch_array($query)) {
			...
			echo '	<input type="submit" name="editMItem" id="editMItem" value="Wijzig" />
					<input type="submit" name="deleteMItem" id="deleteMItem" value="Verwijder" onclick="return confirm(\'Are your sure you want to delete this?\')"/>';
		}
		...
		echo '	<h2 class="h2header">Nieuwe subpagina</h2>
				...
				<input type="text" name="pageTitle" id="pageTitle" placeholder="E.g. Bestuur" />
